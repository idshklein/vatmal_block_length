...
```
## data
We've taken all non-road landuse polygons in vatmal (20622 polygons), and dissolved the vorders in blocks(5606 blocks). this is an example from arara:
...
ggpols2_res %>% bind_cols(pols2) %>% st_sf() %>% st_write("pols.gpkg")
```
...
#   )
ggmor2_res %>% bind_cols(mor2) %>% st_sf() %>% st_write("mor.gpkg")
```
...
#   )
ggch2_res %>% bind_cols(ch2) %>% st_sf() %>% st_write("ch.gpkg")
```
